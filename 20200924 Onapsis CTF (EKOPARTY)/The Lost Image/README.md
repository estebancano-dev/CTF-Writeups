> lost image: Hi there, You don't need to thank me. See you next time. Bests, Hurley. P.S: Apply sha256 to the code.

File: [thelostimage.txt](https://raw.githubusercontent.com/estebancano-dev/CTF-Writeups/master/20200924%20Onapsis%20CTF%20(EKOPARTY)/The%20Lost%20Image/thelostimage.txt "thelostimage.txt")

The file format is

```
1:0slbzoP...
2:AxkhkZc...
3:isoOmQv...
4:/9j/4AA...
5:Fl42qxw...
6:zyN2ymh...
...
```

where we have a number of line, then : and a base64 encoded string

if we decode it all we can see `Created with GIMP` string (line 4). Its the header of the image

I made a script to concatenate every other line to this header and write it to a file, just to see what happens

In PHP
```php
<?php

$cabecera = "";
$handle = fopen("thelostimage.txt", "r");
while (($line = fgets($handle)) !== false) {
	$arre = explode(":", $line);
	$cad = base64_decode($arre[1]);
	file_put_contents($arre[0] . ".jpg", base64_decode($cabecera) . $cad);
}
```


In Python
```python
#!/usr/bin/env python
import base64

cabecera = ""

with open('thelostimage.txt', 'r') as file:
    for l in file:
        arre = l.split(":")
        g = open(f'salida{str(arre[0])}.jpg', 'wb')
        g.write(base64.b64decode(cabecera) + base64.b64decode(arre[1]))
        g.close()
```

In image 8.jpg we can see the flag
TheDharmaInitiative

#### ONA{TheDharmaInitiative} -> ONA{b8cedfbd79c424a8e53b93746c70908c14721363a5e4e29e5731fab661add0aa}